...
function removeFirstOccurrences(str, value) {
  if (str.includes(value)) {
    ...
    const finalStr = str.slice(0, startIndex) + str.slice(endIndex, str.length);
    return finalStr;
  }
  return str;
}
...
function formatTime(minutes, seconds) {
  ...
  const formatMin = min.length === 1 ? min.padStart(2, '0') : min;
  const formatSec = sec.length === 1 ? sec.padStart(2, '0') : sec;
...
function countVowels(str) {
  ...
  let countOfVowels = 0;
  for (let i = 0; i <= str.length; i += 1) {
    if (vowels.includes(str[i])) countOfVowels += 1;
  }
...
function isPalindrome(str) {
  const strChar = str.replace(/\W/g, '').toLowerCase();
  const reverseStrChar = strChar.split('').reverse().join('');
  return strChar === reverseStrChar;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 1; i < words.length; i += 1) {
    longestWord =
      longestWord.length >= words[i].length ? longestWord : words[i];
  }
  return longestWord;
}
...
function reverseWords(str) {
  ...
  const wordsReversed = words.map((element) => {
...
function encodeToRot13(str) {
  const alphUpperCase = 'ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const alphLowerCase = 'abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz';
  let newStr = '';
  for (let i = 0; i < str.length; i += 1) {
    if (alphUpperCase.includes(str[i])) {
      newStr += alphUpperCase[alphUpperCase.indexOf(str[i]) + 13];
    } else if (alphLowerCase.includes(str[i])) {
      newStr += alphLowerCase[alphLowerCase.indexOf(str[i]) + 13];
    } else newStr += str[i];
  }
  return newStr;
}
